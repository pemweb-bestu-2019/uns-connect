        ...
        <div class="tab-content pt-4">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p class="mb-0">Dengan menekan tombol dibawah ini, kamu akan dialihkan ke laman pembayaran. Setelah
                        pembayaran berhasil, maka kamu bisa menjadi anggota dari organisasi ini.</p>
                    <fieldset>
                        <form action="{{ route('organizations.registration.store', $organization->id_organization) }}"
                            method="POST">
                            @csrf
                            <hr>
                            <div class="section-block text-center text-sm-left">
                                <h3 class="section-title"> Pilih salah satu divisi pada daftar </h3>
                                <!-- .visual-picker -->
                                @forelse ($divisions as $division)
                                <div class="visual-picker visual-picker-sm has-peek px-3">
                                    <!-- visual-picker input -->
                                    <input type="radio" id="division-{{ $division->id_division }}"
                                        value="{{ $division->id_division }}" name="division">
                                    <!-- .visual-picker-figure -->
                                    <label class="visual-picker-figure" for="division-{{ $division->id_division }}">
                                        <span class="visual-picker-content"><span class="tile tile-sm bg-info"><i
                                                    class="oi oi-person"></i></span></span>
                                        <!-- /.visual-picker-content --></label> <!-- /.visual-picker-figure -->
                                ...
                                <div class="alert alert-primary has-icon" role="alert">
                                    ...
                                    <p class="mb-0">Jika kamu membaca pesan ini, hubungi pemilik dan memberikan pesan
                                        kesalahan <strong>ERR_DIV_NOTEXISTS</strong>.</p>
                                </div>
                                @endforelse
                            </div>
                            <hr>
                            <div class="section-block text-center text-sm-left">
                                ...
                                <h2>{{ $organization->registration_fee == 0 ? 'Gratis' : 'Rp' . number_format($organization->registration_fee, 0) }}
                                </h2>
                            </div>
                            <hr>
                            <p><button type="submit" class="btn btn-primary" @if(count($divisions)==0) disabled
                                    @endif><i class="far fa-registered"></i> Daftar Organisasi</button></p>
                        </form>
                    </fieldset>
                    @else
                    @if($membership)
                    ...
                    @include('vendor.include.message_flash')
                    <p>Berikut ini adalah informasi dari membership anda di organisasi ini. Kamu bisa menghapus
                        keanggotaan ini kapan saja.</p>
                    <div class="card-body">
                        <dl class="d-flex justify-content-between">
                            ...
                            <dd class="text-right mb-0">
                                <strong><span
                                        class="badge badge-primary">{{ $membership->division->name }}</span></strong>
                            </dd>
                        </dl>
                        <dl class="d-flex justify-content-between">
                            <dt class="text-left">
                    ...
                    <p>Mohon maaf namun organisasi ini tidak menerima pendaftaran untuk saat ini, silahkan hubungi
